...
_, sys_info, digest_auth, cookies = communication.connect_robot_with_ipaddr_def_user('local')
print sys_info

# Checks if rapid simulation has started.
_, response, cookies = rapid_datatypes.get_rapid_data('local', cookies, digest_auth, 'T_ROB1', 'MainModule', 'sim_started')
...
msg, cookies = rapid_speeddata.edit_and_write_rapid_data_base('local', cookies, digest_auth,
                                                              'T_ROB1', 'MainModule', 'speed', 'v100')
print msg
# Finep is set to True in order to not get corner path failure warning
msg, cookies = rapid_zonedata.edit_and_write_rapid_data('local', cookies, digest_auth, 'T_ROB1', 'MainModule', 'zone',
...
while num_flowers < 10:
    while theta < max_degrees:
        got_value, response, cookies = rapid_datatypes.get_rapid_data('local', cookies, digest_auth,
        ...
        if got_value:
            if rapid_bool.get_state(response) == False:
                ...
                msg, cookies = rapid_num.edit_and_write_rapid_data('local', cookies, digest_auth,
                                                                   'T_ROB1', 'MainModule', 'x', x)
                print msg
                msg, cookies = rapid_num.edit_and_write_rapid_data('local', cookies, digest_auth,
                                                                   'T_ROB1', 'MainModule', 'y', y)
                print msg
                msg, cookies = rapid_bool.edit_and_write_rapid_data('local', cookies, digest_auth, 'T_ROB1', 'MainModule', 'new_point', True)
                print msg
    theta = 0
    num_flowers = num_flowers + 1

# Stops the RAPID execution.
_, cookies = rapid_bool.edit_and_write_rapid_data('local', cookies, digest_auth, 'T_ROB1', 'MainModule', 'run', False)
